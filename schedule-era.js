...
/*
  MATRIX TOP-3 · SCHEDULE ERA
  Cutover:
    old archive: №267755 and earlier
    new schedule: №267756 and later
  Main Matrix keeps native frequency/intensity logic and newest dates at the top.
*/
(() => {
  ...
  function eraRows(era){
    return filteredByDays().filter(d => eraOf(d) === era);
  }
  function uniqueTimes(draws){
    return [...new Set(draws.map(d => d.time))].sort((a,b)=>a.localeCompare(b));
  }
  function currentTimes(){
    const t = uniqueTimes(state.draws.filter(d=>d.id >= CUTOVER_ID));
    return t.length ? t : [...OLD_TIMES];
  }

  // LAB / next draw uses the current factual schedule, not the historical one.
  scheduleTimes = function(){ return currentTimes(); };
  ...
  function nativeGrid(era){
    ...
    const times=era==='new' ? uniqueTimes(rows) : OLD_TIMES.slice();
    ...
    html+=era==='new'
      ? `<div class="schedule-era-head"><span class="era-badge">НОВОЕ РАСПИСАНИЕ</span><span class="era-range">№267756 и далее</span><span class="era-sub">LIVE · свежий тираж сверху</span></div>`
  ...
  renderMatrix = function(){
    ...
    const newTimes=uniqueTimes(state.draws.filter(d=>d.id>=CUTOVER_ID));
    $('#matrixStatus').innerHTML=
      `Режим: <b>${state.mode==='ALL'?'ВСЕ':state.mode}</b><br>`+
      `Новая сетка: <b>${newTimes.length?newTimes.join(', '):'ожидаем факты'}</b><br>`+
