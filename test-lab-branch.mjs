...
// Shared server archive must freeze before the draw, then only append the fact/result.
...
for(const needle of ['id="branchForecast"','id="branchArchive"','id="branchArchiveToggle"','id="branchArchiveBody"','Ступени','Три столба','ПОКА НЕАКТИВНО','branch-archive-top3.json','lab-branch-core.js?v=1.8.0','lab-branch.js?v=1.8.0'])assert.ok(html.includes(needle),`index missing ${needle}`);
...
assert.ok(sw.includes("matrix-top3-v1.8.0-lab-branch-server-archive"));
...
console.log(`Core ${Core.VERSION}; D/date, S/time, shared server frozen/archive, mobile S0 card, visual matrix arrows and cache checked.`);